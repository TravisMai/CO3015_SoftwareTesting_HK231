...
class TestNoSubmission():
  ...
  def test_noSubmission(self):
    # Test name: test_noSubmission
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "collapseElement-2")))
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "collapseElement-10")))

    # 19 | click | id=collapseElement-2 | 
    # Check if the user menu is already open
    is_user_menu_open = self.driver.find_element(By.ID, "collapseElement-2").get_attribute("aria-expanded") == "true"
    # Click the toggle only if it's not open
    if not is_user_menu_open:
        self.driver.find_element(By.ID, "collapseElement-2").click()

    # 20 | uncheck | id=id_assignsubmission_onlinetext_enabled | 
    element = self.driver.find_element(By.ID, "id_assignsubmission_onlinetext_enabled")
    if element.is_selected(): element.click()
    
    # 21 | uncheck | id=id_assignsubmission_file_enabled | 
    element = self.driver.find_element(By.ID, "id_assignsubmission_file_enabled")
    if element.is_selected(): element.click()
    

    # 22 | click | id=collapseElement-10 | 
    # Check if the user menu is already open
    ...
    # 23 | click | id=id_completion_2 | 
    self.driver.find_element(By.ID, "id_completion_2").click()
    # 24 | check | id=id_completionsubmit | 
    element = self.driver.find_element(By.ID, "id_completionsubmit")
    ...
    elements = self.driver.find_elements(By.ID, "id_error_completionexpected")
    assert len(elements) > 0 , "Error message not displayed"
